...
view[CCW[xn+3]].ant&&view[CCW[xn+3]].ant.friend&&dOK[CCW[xn+4]]) {return {cell:CCW[xn+4]};} else if (dOK[CCW[xn+3]]) {return {cell:CCW[xn+3]};} else if (dOK[CCW[xn+5]]) {return {cell:CCW[xn+5]};} else {return NOP;}} else if (spcRM()) {return (rUTRRSy());}return (rLostMSy(false));}function rULRL02Sy() {var ptrn;var msm;if (sL[LRM0]>0) {ptrn=PTGRL0;msm=patC(ptrn,AIMR,1,1);}if (xn<0) {ptrn=PTGRL2;msm=patC(ptrn,AIMR,1,1);}if (xn>=0) {if (view[CCW[xn+3]].ant&&view[CCW[xn+3]].ant.friend&&dOK[CCW[xn+4]]) {return {cell:CCW[xn+4]};} else if (dOK[CCW[xn+3]]) {return {cell:CCW[xn+3]};} else if (dOK[CCW[xn+5]]) {return {cell:CCW[xn+5]};} else {return NOP;}} else if (spcRM()) {return (rUTRRSy());}return (rLostMSy(false));}function rULRR0Sy() {var ptrn=PTGRR0;msm=patC(ptrn,AIML,2,1);if (xn>=0) {return (runUMLeaveRRTactic());} else if (spcRM()) {return (rUTRRSy());}return (rLostMSy(false));}function rULRR2Sy() {var ptrn=PTGRR2;msm=patC(ptrn,AIML,2,1);if (xn>=0) {return (runUMLeaveRRTactic());}    return (rLostMSy(false));}function rUPSSy() {var ptrn=PTGRR1;var msm=patC(ptrn,AIMD,3,2);if (xn>=0) {var c=CCW[xn+1];if (view[c].ant&&view[c].ant.friend&&(view[c].ant.food>0)) {
...
}}if (dOK[CCW[xn+5]]) {return {cell:CCW[xn+5]};} else if (dOK[CCW[xn+6]]) {return {cell:CCW[xn+6]};} else {return NOP;}}return (rLostMSy(false));}function rLTRRSy() {return (rLCRTc());}function rLFRSy() {return NOP;}function rLRHSy() {var ptrn=PTFRL1G;var msm=patC(ptrn,AIMR,0,1);if (xn>=0) {return (rLRLTc());}for (var i=0; i<TN; i++) {if (view[CCW[i]].ant&&view[CCW[i]].ant.friend&&
(view[CCW[i]].ant.type==ASF)) {
if (dOK[CCW[i+1]]) {return {cell:CCW[i+1]};}}}return NOP;}function rLLRL1Sy() {var ptrn=PTGRL1;var msm=patC(ptrn,AIMR,0,1);if (xn>=0) {return (rLRLTc());} else if (spcRM()) {return (rLTRRSy());}return (rLostMSy(false));}function rLLRL02Sy() {var ptrn;var msm;if (sL[LRM0]>0) {ptrn=PTGRL0;msm=patC(ptrn,AIMR,1,1);}if (xn<0) {ptrn=PTGRL2;msm=patC(ptrn,AIMR,1,1);}if (xn>=0) {return (rLRLTc());}return (rLostMSy(false));}function rLLRR0Sy() {var ptrn=PTGRR0;msm=patC(ptrn,AIML,2,1);if (xn>=0) {return (rLRRTc());} else if (spcRM()) {return (rLTRRSy());}return (rLostMSy(false));}function rLLRR2Sy() {var ptrn=PTGRR2;msm=patC(ptrn,AIML,2,1);if (xn>=0) {return (rLRRTc());} else if (spcRM()) {return (rLTRRSy());}return (rLostMSy(false));}function rLCRSy() {for (var i=TN; i>=1; i--) {if (!dOK[CCW[i]]&&dOK[CCW[i-1]]) {
